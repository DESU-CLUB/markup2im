...
DISABLE_WANDB_IF_DDP = torch.cuda.device_count() > 1
...
def process_args(args):
    ...
    parser.add_argument('--latent_scaling_factor', default=0.2, type=float, help=("I don't know what the fuck I am doing"))
...
def train(train_dataloader, save_dir, save_model_every, \
    ...
    ### We want to randomly sample a batch of images and formulas for eval here:
    eval_batch = sample_random_batch(train_dataloader)
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.is_main_process:
                wandb.log({'scheduled_sampling': disp_str}, step=global_step)
            ...
            global_step += 1
        if epoch % save_model_every == 0 and accelerator.is_main_process:
            save_model(image_decoder, os.path.join(save_dir, f'model_e{num_epochs}_lr{learning_rate}.pt.{epoch}'))
            if not DISABLE_WANDB_IF_DDP:
                with torch.no_grad():
                    image_decoder.eval()
                    pipeline = DDPMPipeline(unet=image_decoder, scheduler=noise_scheduler)
                    clean_images = eval_batch['images'].to(accelerator.device)
                    input_ids = eval_batch['input_ids'].to(accelerator.device)
                    masks = eval_batch['attention_mask'].to(accelerator.device)
                    encoder_hidden_states = encode_text(text_encoder, input_ids, masks)
                    bs = clean_images.shape[0]
                    noisy_images_t = torch.randn(clean_images.shape).to(clean_images.device) #[16,32,32]
                    noisy_images = []
                    clean_images_log = []
                    ...
                    for filename, pred_image, clean_image in zip(eval_batch['filenames'], pred_images, clean_images):
                        noisy_images.append(wandb.Image(pred_image, caption=filename))
                        clean_images_log.append(wandb.Image(clean_image, caption=filename))
    ...
    if accelerator.is_main_process and DISABLE_WANDB_IF_DDP:
        with torch.no_grad():
                ...
                encoder_hidden_states = encode_text(text_encoder, input_ids, masks)
                bs = clean_images.shape[0]
                noisy_images_t = torch.randn(clean_images.shape).to(clean_images.device)
                noisy_images = []
                clean_images_log = []
                for _, pred_images in pipeline.run_clean(
                    batch_size = bs,
